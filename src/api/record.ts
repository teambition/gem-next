...
import { RecordQueryBll, FindByIdQuery } from '../interface/record-query'
...
export class RecordAPI {
  ...
  @after(resultMW())
  async getById({ id, limit = 10, skip = 0, options }: FindByIdQuery) {
    const resp = await this.recordBll.findById({
      id,
      limit,
      skip,
      options,
